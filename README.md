...
GENERATE(R, C, N):
  repeat:
    ...
    # 3. Greedy clue deletion to a fixpoint, guarded by both oracles.
...
machine-checked certificate of a guess-free solving path, and the fixpoint
deletion leaves an irredundant clue set: remove any clue and the puzzle
breaks. Runtime is negligible at pen-and-paper scales (milliseconds for
5×5, ~0.5 s for 7×7 with 12 breaks).

---

## Worked example (5×5, N = 4)

Generated by `python3 firebreak.py --demo` (instance `seed=12`; the program
...
Reproduce with `python3 firebreak.py --generate 7 7 12 --seed 12002`
(verified unique and guess-free; 17 clues). One cell burns at minute 11 —
it is 5 steps from the ★.
